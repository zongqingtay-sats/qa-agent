...
export function FlowBlockNode({ data, selected }: { data: BlockData & { executionStatus?: string }; selected: boolean }) {
  ...
  return (
    ...
    >
      ...
      {data.blockType === "navigate" && data.url && (
        <p className="text-xs mt-1 font-mono opacity-60 truncate">{data.url}</p>
      )}
      {data.blockType === "set-variable" && data.variableName && (
        <p className="text-xs mt-1 font-mono opacity-60 truncate">@{data.variableName}</p>
      )}
      {data.blockType !== "navigate" && data.blockType !== "set-variable" && data.selector && (
