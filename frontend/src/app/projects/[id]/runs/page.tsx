/**
 * Project-scoped test runs page.
 *
 * Shows all test runs for test cases belonging to this project,
 * with search, selection, and export features.
 */
...
import { projectsApi, testRunsApi, exportApi } from "@/lib/api";
import type { TestRunListItem, TestRunDetail } from "@/types/api";
import { runTestCase } from "@/lib/run-test";
import { useSSE, type SSEEvent } from "@/hooks/use-sse";
import { toast } from "sonner";
import { TestRunTable } from "@/app/test-runs/_components/test-run-table";
...
export default function ProjectRunsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [projectCaseIds, setProjectCaseIds] = useState<Set<string>>(new Set());
  ...
  async function loadData() {
    try {
      ...
      const caseIds = new Set(casesRes.data.map((tc) => tc.id));
      setProjectCaseIds(caseIds);
      setTestRuns(runsRes.data.filter((r) => caseIds.has(r.testCaseId)));
    } catch { /* ignore */ }
  ...
  useSSE({
    ...
    onEvent: useCallback((event: SSEEvent) => {
      ...
      if (event.type === "test-run:created" && projectCaseIds.has(data.testCaseId)) {
